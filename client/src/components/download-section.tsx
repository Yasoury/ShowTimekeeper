import { useLanguage } from '@/hooks/use-language';
import { CameraLogo } from './camera-logo';
import { Shield, Smartphone, Globe } from 'lucide-react';

export function DownloadSection() {
  const { t, isRTL } = useLanguage();

  return (
    <section id="download" className="py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto text-center">
        <CameraLogo size="lg" className="mx-auto mb-6 animate-pulse-slow" />
        
        <h2 className="text-3xl md:text-4xl font-bold mb-4">
          {t('readyTitle').split(' ').slice(0, -1).join(' ')}{' '}
          <span className="gradient-text">{t('readyTitle').split(' ').slice(-1)}</span>
        </h2>
        
        <p className="text-xl text-muted-foreground mb-8">
          {t('readySubtitle')}
        </p>

        <div className="flex flex-col sm:flex-row gap-6 justify-center mb-8">
          <a 
            href="https://apps.apple.com/us/app/show-timekeeper/id6446911407"
            className="download-btn flex items-center justify-center px-8 py-4 rounded-2xl text-primary-foreground font-medium text-lg group"
            data-testid="button-download-ios-main"
          >
            <svg className="w-8 h-8 mr-4 group-hover:scale-110 transition-transform" viewBox="0 0 24 24" fill="currentColor">
              <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
            </svg>
            <div className={`text-${isRTL ? 'right' : 'left'}`}>
              <div className="text-sm opacity-80">{t('downloadOnThe')}</div>
              <div className="text-lg font-semibold">{t('appStore')}</div>
            </div>
          </a>

          <a 
            href="https://play.google.com/store/apps/details?id=com.bhaar.timetracker&hl=en"
            className="download-btn flex items-center justify-center px-8 py-4 rounded-2xl text-primary-foreground font-medium text-lg group"
            data-testid="button-download-android-main"
          >
            <svg className="w-8 h-8 mr-4 group-hover:scale-110 transition-transform" viewBox="0 0 24 24" fill="currentColor">
              <path d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.53,12.9 20.18,13.18L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z"/>
            </svg>
            <div className={`text-${isRTL ? 'right' : 'left'}`}>
              <div className="text-sm opacity-80">{t('getItOn')}</div>
              <div className="text-lg font-semibold">{t('googlePlay')}</div>
            </div>
          </a>
        </div>

        <div className={`flex items-center justify-center space-x-8 text-sm text-muted-foreground ${isRTL ? 'space-x-reverse' : ''}`}>
          <div className="flex items-center">
            <Shield className="w-4 h-4 text-primary mr-2" />
            <span>{t('safeSecure')}</span>
          </div>
          <div className="flex items-center">
            <Smartphone className="w-4 h-4 text-accent mr-2" />
            <span>{t('iosAndroid')}</span>
          </div>
          <div className="flex items-center">
            <Globe className="w-4 h-4 text-primary mr-2" />
            <span>{t('bilingual')}</span>
          </div>
        </div>
      </div>
    </section>
  );
}
